...
function SinglPic() {
  ...
  const SelectedPictureDate = (FavSecMode==ture ? Database.PersonalInfo.Picture.Favorite : Database.PersonalInfo.Picture.AllPicture);
  ...
  return (
    <div className='bg-black'>
      {(
        ...
        > 
          <Lightbox
            ...
            onCloseRequest={() => {navigate(`/life/Gallery`);}}
            onMovePrevRequest={() => {navigate(`/life/Gallery/${prevIndex}`);}}
            onMoveNextRequest={() => {navigate(`/life/Gallery/${nextIndex}`);}}
            closeLabel={"string"}
            animationOnKeyInput={true}
            imageTitle={(lang === "0" ? "Personal Gallery " : (lang === "1" ? "个人图库 " : "")) +" [ " +picture +" / " +totalPictures+" ]"}
      ...
      <div className='hidden'>
        <img src={SelectedPictureDate[(nextIndex+1)% totalPictures].replace(/\.(png|jpg|jpeg|gif|bmp|svg|webp)$/, ".md.$1")} alt='prevThumbnail' />
        <img src={SelectedPictureDate[(nextIndex+2)% totalPictures].replace(/\.(png|jpg|jpeg|gif|bmp|svg|webp)$/, ".md.$1")} alt='prevThumbnail' />
