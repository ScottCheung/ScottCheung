...
function GridHouseCard() {
  ...
  const nextIndex = reverseOrder
    ? (index - 1 + totalPictures) % totalPictures
    : (index + 1) % totalPictures;
  const prevIndex = reverseOrder
    ? (index + 1) % totalPictures
    : (index - 1 + totalPictures) % totalPictures;
        ...
        <div className='mt-24 px-[20px] w-[100%]'>
          <AnimatePresence>
            ...
            >
              <motion.label
                layout
                className='flex-col inline-flex relative  cursor-pointer mb-12 lg:z-50'
              >
                {!lightboxIsOpen && (
                ...
                {picturesToDisplay.map((item, index) => (
                  <AnimatePresence>
                    ...
                    >
                      ...
                      <div
                        role='status'
                        className=' flex -z-10  items-center justify-center w-full h-full bg-gray-300/30 rounded-lg  animate__faster dark:bg-gray-700/30'
                        style={{
                      ...
                      >
                        <svg
                          className='w-36 h-36 text-gray-200 dark:text-gray-600 animate-pulse'
                          aria-hidden='true'
