...
function GridHouseCard() {
  ...
  const MaxScale = 5;
  const MinScale = 1;
  const picturesDate = (FavSecMode ? Database.PersonalInfo.Picture.Favorite : Database.PersonalInfo.Picture.AllPicture);
  const [loadedImages, setLoadedImages] = useState(42);
  const [imageLoaded, setImageLoaded] = useState(false);
  const [selectedImage, setSelectedImage] = useState(null);
  const [currentImage, setcurrentImage] = useState(localStorage.getItem("currentImage")||null);
  const [zoomLevel, setZoomLevel] = useState(1);
  const picturesToDisplay = reverseOrder
    ? picturesDate.slice(-loadedImages).reverse()
    : picturesDate.slice(0, loadedImages);
  const totalPictures = picturesDate.length;
  const [zIndexes, setZIndexes] = useState(Array.from({ length: totalPictures }, (_, index) => index + 1));
  const { gallery } = useParams();
  const index = parseInt(gallery);
  const nextIndex = (reverseOrder ? ((index - 1 + totalPictures) % totalPictures) : ((index + 1) % totalPictures));
  const prevIndex = (reverseOrder ? ((index + 1) % totalPictures) : ((index - 1 + totalPictures) % totalPictures));
  ...
  const toolbarButtons = [
    <button key="originalButton"
      className=' border-white px-4 text-white hover:bg-gray-700 hover:text-white'
      onClick={handleOriginalImageClick}>{lang === "0" ? "Raw picture" : (lang === "1" ? "原图 " : "")}</button>,
  ];
  // 放大图片
  const zoomIn = () => {
      ...
      <div className={`flex justify-between w-[100%]`}>
        <div className='mt-24 px-[20px] w-[100%]'>
          <AnimatePresence>
              ...
              <div className=" items-end lg:flex justify-between">
                ...
                <div className='text-right flex-col flex'>
                  <h1 className="text-[27px] font-[500]  animate__animated animate__fadeInRight">
                    {lang == "0" && (FavSecMode ? "Favorite Pictures" : "All Pictures")}
                    {lang == "1" && (FavSecMode ? "精选图片" : "全部图片")}

                  </h1>
                  <p className='animate__animated animate__fadeInLeft text-[20px] flex justify-end'>
                    <i className='fi fi-sr-picture mr-3 pt-1'></i><N className="w-[200px]" n={totalPictures} d={1} />
                  </p>
            ...
            <motion.div layout className="w-[100%] px-auto flex justify-between lg:z-50 sticky top-[12px] mt-[20px] h-0 mb-[50px]">
              ...
              {!lightboxIsOpen&& <div className='inline-flex items-center'>
                  {/* {con} */}
                  <div className={`animate__animated animate__fadeInLeft w-20 h-12   flex  items-center rounded-full p-2 cursor-pointer ${!reverseOrder == true ? 'justify-end bg-gradient-to-r from-sky-500 to-emerald-500' : 'justify-start bg-gray-300'}`} onClick={() => { setReverseOrder(!reverseOrder); localStorage.setItem('reverseOrder', !reverseOrder) }}>
                    <motion.div className={`w-8 h-8 bg-white/50 rounded-full`} layout transition={spring} />
                  </div>
                  <span className="animate__animated animate__fadeInRight ms-3 text-[15px] font-medium text-gray-900 dark:text-gray-200">{(lang == "0" ? "Sequential" : "时间正序")}</span>
                </div>}
              </motion.label>
              <motion.label layout className="flex-col inline-flex  relative   cursor-pointer mb-12 lg:z-50">
              {!lightboxIsOpen&&<div className='inline-flex items-center'>
                  <span className="animate__animated animate__fadeInLeft mr-3 text-[15px] font-medium text-gray-900 dark:text-gray-200">{(lang == "0" ? "Favorite" : "精选")}</span>
                  <div
                    className={`animate__animated animate__fadeInRight w-20 h-12 flex items-center rounded-full p-2 cursor-pointer ${FavSecMode == true
                ...
                {picturesToDisplay.map((item, index) => (
                  <AnimatePresence>
                    <motion.div
                      ...
                      whileTap={{ scale: 0.95 }}
                      exit={{ opacity: 0, scale: 0 ,transition: { duration: 0.7 }}}
                    ...
                    >
                      <motion.div
                        ...
                        whileHover={{ scale: 1.2, rotate: 2, transition: { duration: 1 } }}
                        ...
                        onClick={() => {
                          ...
                          setLightboxIsOpen(true);
                          navigate(`/life/gallery/${reverseOrder ? (totalPictures - index) : (index + 1)}`);
                          timeoutId = setTimeout(() => {
                            document.body.style.overflow = 'hidden';
                          }, 500)

                        }}
                        style={{ animationDelay: `${0.05 * (index % 36)}s`, zIndex: zIndexes[index] }}
                      ...
                      >
                        <motion.img
                          ...
                          className=" absolute w-full h-full object-cover -z-1 " src={item.replace(/\.(png|jpg|jpeg|gif|bmp|svg|webp)$/, ".md.$1")} alt={item} />
            ...
            <AnimatePresence>
              {lightboxIsOpen == true &&
                <motion.div
                  ...
                  className={'  fixed top-0 left-0 w-screen h-screen z-50 bg-gray-950/90 bg-blur-md'}
                >
                  <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                  ...
                  >
                    <motion.img
                      ...
                      onClick={() => {
                        navigate(`/life/gallery`); setLightboxIsOpen(false);
          ...
          layoutId={selectedImage}
            className='fixed top-0 left-0 transition-all w-full h-full object-contain z-10'
            src={item} alt={item.replace(/\.(png|jpg|jpeg|gif|bmp|svg|webp)$/, ".md.$1")} /> */}

                    {/* left picture */}
                    <CtButton type={">"}
                      key={"left"}
                      position={"top-[50%] left-[30px]"} btnsize={"50px"} icon={"w-[16px] h-[16px] rotate-180"} className="z-50 "
                      onClick={() => {
                        setLightboxIsOpen(true);
                        resetZoom(); 
                    ...
                    <CtButton type={">"}
                      key={"right"}
                      position={"top-[50%] right-[30px]"} btnsize={"50px"} icon={"w-[16px] h-[16px] rotate-0"} className="z-50 "
                      onClick={() => {
                        setLightboxIsOpen(true);
                        resetZoom(); 
                    ...
                    <CtButton type={"x"}
                      key={"close"}
                      position={"top-[20px] right-[30px]"} btnsize={"30px"} icon={"w-[10px] h-[10px] rotate-0"} className="z-50"
                      onClick={() => {
                        navigate(`/life/gallery`); setLightboxIsOpen(false);
                        document.body.style.overflow = 'auto';
                        resetZoom(); 
                    ...
                    />
                    {/* zoomIn Button */}
                    <CtButton type={"zoomIn"}
                        key={"zoomIn"}
                        position={"top-[20px] right-[150px]"} btnsize={"30px"} icon={"w-[16px] h-auto rotate-0"} className="z-50"
                        onClick={zoomIn}
                        disabled={zoomLevel === MaxScale}
                      />
                    {/* ZoomOut Button */}
                    <CtButton type={"zoomOut"}
                      key={"zoomOut"}
                      position={"top-[20px] right-[90px]"} btnsize={"30px"} icon={"w-[16px] h-auto rotate-0"} className="z-50"
                      onClick={zoomOut}
                      disabled={zoomLevel === MinScale}
                    />
                    {/* Recovery Button */}
                    <CtButton type={"recovery"}
                      key={"Recovery"}
                      position={"top-[20px] right-[210px]"} btnsize={"30px"} icon={"w-[16px] h-[16px] rotate-0"} className="z-50"
                      onClick={resetZoom}
          ...
          />  */}
              ...
              {nextIndex && <div className='hidden'>
                {Array.from({ length: 6 }, (_, i) => (
                  <img key={i} src={picturesDate[(nextIndex + i + 1) % totalPictures].replace(/\.(png|jpg|jpeg|gif|bmp|svg|webp)$/, ".th.$1")} alt={`preload-${i}`} />
                ))}
                {Array.from({ length: 6 }, (_, i) => (
                  <img key={i} src={picturesDate[(nextIndex + i + 1) % totalPictures]} alt={`preload-${i}`} />
                ))}
              </div>}
            </AnimatePresence>
          </AnimatePresence>
