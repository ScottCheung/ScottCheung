...
const Toast = ({ type, message, duration = 5000 }) => {
  ...
  return (
    ...
    {isVisible && (
        ...
        className={` flex items-center w-[350px] h-auto p-[28px] mb-4  rounded-[14px] shadow-[28px]  ${type === 'success' ? 'bg-green-100' : type === 'error' ? 'bg-red-100' : 'bg-orange-100'}`}
        role="alert"
      >
        <div className='flex flex-col'>
          <div className={`flex justify-between text-${type === 'success' ? 'green' : type === 'error' ? 'red' : 'orange'}-500 bg-${type === 'success' ? 'green' : type === 'error' ? 'red' : 'orange'}-100 rounded-lg dark:bg-${type === 'success' ? 'green' : type === 'error' ? 'red' : 'orange'}-800 dark:text-${type === 'success' ? 'green' : type === 'error' ? 'red' : 'orange'}-200`}>
              ...
              {/* success */}
               {type === 'success' ? (
                  ...
                  <svg className="w-16 h-16" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                ...
                <div className='flex flex-col justify-start ml-[10px]'>
                  <span className=' text-[17px] font-black'>
                    ...
                    {lang==0 && (type === 'success' ? 'Congradulation' : type === 'error' ? 'Error' : 'Attention') }
                    {lang==1 && (type === 'success' ? '恭喜您' : type === 'error' ? '错误' : '请注意') }
                    {` ( ${countdown} s  )`}
                  </span>
                  <div className={`${type === 'success' ? 'text-green-900' : type === 'error' ? 'text-red-900' : 'text-orange-900'} text-[14px] font-normal`}>{message}</div>
                </div>
...
const showToast = (type, message, duration, offset) => {
  ...
  ReactDOM.render(<Toast type={type} message={message} duration={duration} />, div);
