...
export default function Gobelldesign() {
  ...
  // 处理 iframe 的高度
  const handleIframeLoad = (index, e) => {
  ...
  return (
    <div className='flex p-3 bg-gray-100'>
      ...
      <div className='pt-[140px] flex flex-col w-screen h-full overflow-x-hidden items-center justify-start'>
        ...
        {Object.entries(iframeHeight).map(([index, height]) => (
          <div key={index} className='p-[20px] text-black'>
            {height}
          </div>
        ))}
          ...
          {posts.map((post, index) => (
            ...
            >
              <iframe
                ...
                style={{
                  width: '100%',
                  height: iframeHeight[index] || 'auto',
                }}
                className='flex w-[100%] p-0 scrollbar-hide'
                onLoad={(e) => handleIframeLoad(index, e)}
              ></iframe>
            </div>
          ))}
        </div>
