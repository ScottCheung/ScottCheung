...
function Highquality() {
  ...
  const content = [
    {
      title: lang == 0 ? 'Perfect For Everything' : '追求极致，成就卓越',
      text:
        lang == 0 ?
          SelectText(
            'I believe in setting the highest standards for myself. Whether it’s [coding efficiently], [crafting visually stunning designs], or [enhancing user experiences], I strive for perfection in every detail. Excellence isn’t a goal; it’s my habit.',
            'yellow-300',
            '\t ',
          )
        : SelectText(
            '我相信为自己设定最高标准。无论是[高效编程]、[设计美观界面]还是[优化用户体验]，我都在每一个细节上追求极致。卓越不是目标，而是习惯。',
            'yellow-300',
          ),
    },
    {
      title: lang == 0 ? 'Always Can-Do Attitude' : '永不言败，迎难而上',
      text:
        lang == 0 ?
          SelectText(
    ...
    {
      title: lang == 0 ? 'Result-Oriented' : '专注成果，追求卓越',
      text:
        lang == 0 ?
          SelectText(
    ...
    {
      title: lang == 0 ? 'Empathy and Collaboration' : '共情协作，携手共进',
      text:
        lang == 0 ?
          SelectText(
