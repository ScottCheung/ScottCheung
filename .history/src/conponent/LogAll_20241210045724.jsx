...
const LogEntry = ({ entry, versionType }) => (
  ...
  >
    ...
    <div className='flex flex-col pl-[30px] lg:pl-[50px] gap-y-[20px]'>
        ...
        {versionType &&
          versionType.map((tag, index) => (
            <span
              key={index + tag + index}
              className={`${
                tag === 'Latest' ? 'bg-green-100 text-green-800'
                : tag === 'Patch' ? `bg-orange-100 text-orange-800`
                : tag === 'Minor' ? 'bg-green-100 text-green-800'
                : tag === 'Major' ? 'bg-purple-100 text-purple-800'
                : `bg-sky-100 text-sky-800`
              } inline-flex text-[12px] bg-blue-100  font-medium px-2.5 py-0.5 rounded `}
            >
              {tag}
            </span>
          ))}
      ...
      <header className='mb-4 text-[15px] lg:text-[17px] font-normal text-gray-900 darrk:text-gray-400'>
        {entry.header[lang]}
      </header>
